...
from PyroUbot import OWNER_ID, OWNER_NAME, OWNER_LINK, BOT_FOOTER
import psutil
...
async def _(client, inline_query):
 ...
 ᚗ start_uptime: {uptime}</blockquote>
         <blockquote><b>{BOT_FOOTER}</b></blockquote>
 """
...
async def user_help_inline(client, inline_query):
    ...
    msg = f"<blockquote><b>🚀 ᴍᴇɴᴜ ɪɴʟɪɴᴇ Ibra Decode 🚀\n\n📊 ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}\n⚙️ ᴘʀᴇꜰɪx: {' '.join(SH)}\n\n👨‍💻 ᴜsᴇʀʙᴏᴛ ʙʏ : [{OWNER_NAME}]({OWNER_LINK})\n🛠️ ᴅᴇᴠᴇʟᴏᴘᴇʀ : [Ibra Decode](t.me/ibradecode)\n\n🎯 Klik modul untuk melihat bantuan!</b></blockquote>"
    results = [InlineQueryResultArticle(
        title="🆘 Bantuan & Fitur Bot 🆘",
...
async def close_usernya(client, callback_query):
    if not callback_query.inline_message_id:
        return await callback_query.answer("Error: Inline message ID not found.", show_alert=True)
    unPacked = unpackInlineMessage(callback_query.inline_message_id)
    for x in ubot._ubot:
...
async def help_callback(client, callback_query):
    ...
    top_text = f"<blockquote><b>🚀 ᴍᴇɴᴜ ɪɴʟɪɴᴇ Ibra Decode 🚀\n\n📊 ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}\n⚙️ ᴘʀᴇꜰɪx: {' '.join(SH)}\n\n👨‍💻 ᴜsᴇʀʙᴏᴛ ʙʏ : [{OWNER_NAME}]({OWNER_LINK})\n🛠️ ᴅᴇᴠᴇʟᴏᴘᴇʀ : [Ibra Decode](t.me/ibradecode)</b></blockquote>"
